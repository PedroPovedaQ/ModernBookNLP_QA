...
def main(args):
    ...
    if args.mode == "full" or args.mode == "gold_window":
        # bookcoref: DatasetDict = load_dataset("sapienzanlp/bookcoref", split='test')  # type: ignore
        bookcoref: DatasetDict = load_dataset("/data/bookcoref")#, split='test')  # type: ignore
    ...
    gold = {k['doc_key'] : k for k in gold}
    predicted = {k['doc_key'] : k for k in predicted}
...
def evaluate(gold_elements_list, predicted_elements_list):
    ...
    for k in gold_elements_list:
        gold_doc_elem, pred_doc_elem = gold_elements_list[k], predicted_elements_list[k]
