...
Nineteen focused tests pass, including a regression that first reproduced polling blocked behind a queue writer. The updated Linux ARM64 CPU image builds. First uncached container startup exhausted the former 900-second initialization deadline; startup now allows 1800 seconds and logs elapsed initialization and failure type. This timeout increase is not a performance claim. The updated image’s nine service module hashes match the reviewed source. Both GitHub Actions runs pass commit a958135. The queued HTTP job survived container recreation. Real container inference and completed-result restart smoke remain unverified: the 32 GiB Mac reported about 57 GiB swap in use while loading. The test container was stopped to avoid adding pressure; its named volume and cached models are retained. Native real-model HTTP checks already passed. Repeat container inference on a host with available memory before deployment acceptance.
